...
struct CEEngine {
    ...
    Archetype::Engine* m_arch;             // +0x0c  engine archetype
...
float CEEngine::GetCruiseChargeTime() const { return m_arch->cruise_charge_time; }
float CEEngine::GetCruiseChargeTimeElapsed() const { return m_cruise_charge_elapsed; }
float CEEngine::GetCruisePowerDraw() const { return m_arch->cruise_power_draw; }
float CEEngine::GetLinearDrag() const { return m_arch->linear_drag; }
float CEEngine::GetCruiseDrag() const {
    float base = m_arch->linear_drag;
    return base + EngineEquipConsts::CRUISE_DRAG;
}
float CEEngine::GetThrust(float throttle) const {
    if (throttle > 0.0f)
        throttle *= m_arch->cruise_speed;
    return throttle * m_arch->max_force;
}
